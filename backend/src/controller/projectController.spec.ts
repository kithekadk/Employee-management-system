...
import { deleteProject, fetchOneProject, fetchProjects, updateProject } from "./projectController";
...
describe("Testing projects controller", () => {
    ...
    it('delete a projects', async () => {
        ...
        jest.spyOn(mssql, 'connect').mockResolvedValueOnce({
            request: jest.fn().mockReturnThis(),
            input: jest.fn().mockReturnThis(),
            query: jest.fn().mockResolvedValueOnce({ recordset: expectedoutcome })
        } as never)
        await deleteProject(req as any, res as Response)
        expect(res.status).toHaveBeenCalledWith(200)
        expect(res.json).toHaveBeenCalledWith({ projects: expectedoutcome })
    })
    it('update a project', async () => { 
        ...
        await updateProject(req as any, res as Response)
        expect(res.status).toHaveBeenCalledWith(200)
        expect(res.json).toHaveBeenCalledWith({ projects: expectedoutcome })
    })
}) 